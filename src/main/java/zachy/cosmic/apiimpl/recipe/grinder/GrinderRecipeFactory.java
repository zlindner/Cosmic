...
public class GrinderRecipeFactory {
    ...
    public IGrinderRecipe create(JsonContext context) {
        ...
        if (outputsArray.size() != 3) {
            throw new JsonSyntaxException("Expected 3 outputs, got " + outputsArray.size() + " output(s)");
        }

        NonNullList<ItemStack> outputs = NonNullList.withSize(3, ItemStack.EMPTY);
